...
int main(int argc, char **argv) {
    Struct* cvs;
    cvs = init_Struct();
    ros::init(argc, argv, "fsm_homologation");
    ros::NodeHandle n;
    //ros::Subscriber sub1 = n.subscribe("team", 1, Callback_team);
    //ros::Subscriber sub2 = n.subscribe("localisation_lidar", 1, Callback_lidar);
    //ros::Subscriber sub2 = n.subscribe("dyna_feedback", 1, Callback_feedback);
    ros::Subscriber sub2 = n.subscribe("blackline", 1, Callback_follow);
    //ros::Subscriber sub2 = n.subscribe("opponent", 1, Callback_follow);

    //ros::Publisher pub = n.advertise<std_msgs::int8>("move", 1);
	//ros::Publisher pub = n.advertise<geometry_msgs::Pose2D>("target", 1);
