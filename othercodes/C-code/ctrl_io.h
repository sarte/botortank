...
typedef struct CtrlIn
{
	 ...
	 * the round part).
	 */
	double wheel_1_speed; ///< right wheel speed [rad/s]
	double wheel_2_speed; ///< left wheel speed [rad/s]
	double wheel_3_speed; ///< right wheel speed [rad/s]
	double wheel_4_speed;
...
typedef struct CtrlOut
{
	 ...
	 *    100 corresponds to +0.95*24 V
	 */
	double wheel_commands[4]; ///< wheel motors (R_ID or L_ID) commands [-], bounded in [-100 ; 100]
